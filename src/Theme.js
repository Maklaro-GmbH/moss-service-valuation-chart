class Theme {
    ...
    axesTitles() {
        ...
        this.chart.Scale.prototype._drawTitle = function () {
            ...
            let scaleLabelFontColor = valueOrDefault$a(scaleLabel.fontColor, core_defaults.global.defaultFontColor);
            ...
            if (datasets && datasets.length) {
                this.chart.legend.legendItems.map((item, i) => {
                    if (datasets[i] !== undefined) {
                        if (item.lineDash === undefined && datasets[i].borderDash !== undefined) {
                            item.lineDash = datasets[i].borderDash
                        }
                        if (item.lineDashOffset === undefined && datasets[i].borderDashOffset !== undefined) {
                            item.lineDashOffset = datasets[i].borderDashOffset
                        }
                        if (item.lineCap === undefined && datasets[i].borderCapStyle !== undefined) {
                            item.lineCap = datasets[i].borderCapStyle
                        }
                        if (item.pointStyle === undefined && datasets[i].pointStyle !== undefined) {
    ...
    legendLabels() {
        ...
        this.chart.Legend.prototype.draw = function () {
            ...
            let ctx = me.ctx;
            let fontColor = valueOrDefault$e(labelOpts.fontColor, globalDefaults.defaultFontColor);
            ...
            me.legendItems.map((item, i) => {
                ...
                if (i === 0 || lineWidths[lineWidths.length - 1] + width + (2 * labelOpts.padding) > me.minSize.width) {
            ...
            let drawLegendBox = function(x, y, legendItem) {
                ...
                let lineWidth = valueOrDefault$e(legendItem.lineWidth, lineDefault.borderWidth);
            ...
            let alignmentOffset = function(dimension, blockSize) {
                switch (opts.align) {
                    ...
                    case 'end':
                        let offset = dimension - blockSize - labelPaddingRight - boxWidth * Math.SQRT2
            ...
            helpers$1.each(me.legendItems, function(legendItem, i) {
                ...
                if (i > 0 && x + width + labelOpts.padding + labelPaddingLeft > me.left + me.minSize.width) {
