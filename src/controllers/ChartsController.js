...
class ChartsController {
  ...
  validateLabelsArray(labelsArray) {
    if (!typeCheckers.isArray(labelsArray))
  ...
  validateTicksConfig(ticksConfig, containingAxisPath) {
    ...
    typeCheckers.checkRequiredParamsArePassed(
      ticksConfig,
  ...
  validateYAxisConfig(yAxisConfig, containingDatasetPath) {
    ...
    if (
      !typeCheckers.isInEnumsRange(yAxisConfig.position, Y_AXIS_POSITION_ENUMS)
    )
  ...
  validateDatasetsArray(datasetsArray) {
    if (!typeCheckers.isArray(datasetsArray))
      throw messages.invalidType('data.datasets', 'array')
    datasetsArray.forEach((dataset, index) =>
      this.validateDataset(dataset, index)
    )
  }
  ...
  validateStylingObject(stylingObject) {
    ...
    typeCheckers.checkRequiredParamsArePassed(
      stylingObject,
      REQUIRED_STYLING_PARAMS,
      'styling'
    )
    if (
      !fontsList.some(({ fontName }) => stylingObject.fontFamily === fontName)
    )
  ...
  validatePayload(payload) {
    ...
    if (!typeCheckers.isNumber(payload.width))
      throw messages.invalidType('width', 'number')
    if (!typeCheckers.isNumber(payload.height))
